...
//
//        try {
//            con = MysteryJdbc.getConnection(dataSource);
//
...
//    }
    ...
    public void getServersJoin(GrpcGetServersJoinRequest request, StreamObserver<GrpcGetServersJoinResponse> responseObserver) {
        ...
        String SERVERID_JOIN_QUERY = "select m.server_id as member_server_id from tbl_member as m where m.profile_id = ? order by m.join_at asc limit ? offset ?";
        String SERVER_QUERY = "select " +
                ...
                "from tbl_server as s inner join (" + SERVERID_JOIN_QUERY + ") as sm " +
